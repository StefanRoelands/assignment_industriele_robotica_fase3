...
'''
class ProgramBinsRightSM(Behavior):
	'''
	Oppakken van de producten uit de bins met de rechter arm
	'''


	def __init__(self):
		super(ProgramBinsRightSM, self).__init__()
		self.name = 'ProgramBinsRight'
	...
	def create(self):
		...
		_state_machine = OperatableStateMachine(outcomes=['finished', 'failed'], input_keys=['offset_part_right', 'PreGrasp', 'camera_topic', 'camera_frame', 'offset_part_right', 'part_type', 'arm_id_right', 'part_pose'], output_keys=['part_pose_right'])

...
'''
class ProgramShelvesRightSM(Behavior):
	'''
	Oppakken van de producten uit de shelves met de rechter arm
	'''


	def __init__(self):
		super(ProgramShelvesRightSM, self).__init__()
		self.name = 'ProgramShelvesRight'
	...
	def create(self):
		...
		_state_machine = OperatableStateMachine(outcomes=['finished', 'failed'], input_keys=['PreGrasp', 'camera_topic', 'camera_frame', 'offset_part_right', 'part_type', 'arm_id_right', 'arm_id_right', 'part_pose'], output_keys=['part_pose_right'])
		...
		with _state_machine:
			...
			OperatableStateMachine.add('ComputePick',
										...
										remapping={'move_group': 'move_group_right_arm', 'move_group_prefix': 'move_group_prefix', 'tool_link': 'tool_link_right', 'pose': 'pose', 'offset': 'offset_part_right', 'rotation': 'rotation', 'joint_values': 'joint_values', 'joint_names': 'joint_names'})
			...
			OperatableStateMachine.add('GripperOn',
										...
										transitions={'continue': 'Move_right_arm_home2_2', 'failed': 'ComputePick', 'invalid_arm_id': 'failed'},
										autonomy={'continue': Autonomy.Off, 'failed': Autonomy.Off, 'invalid_arm_id': Autonomy.Off},
										remapping={'arm_id': 'arm_id_right'})

			# x:1378 y:653
			OperatableStateMachine.add('Move_right_arm_home2_2',
										SrdfStateToMoveitAriac(),
			...
			OperatableStateMachine.add('Retry_7',
										WaitState(wait_time=0.3),
										transitions={'done': 'Move_right_arm_home2_2'},
										autonomy={'done': Autonomy.Off})
			...
			OperatableStateMachine.add('Pose',
										...
										autonomy={'done': Autonomy.Off},
										remapping={'value': 'part_pose', 'result': 'part_pose_right'})
